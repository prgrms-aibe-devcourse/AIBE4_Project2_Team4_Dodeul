// src/main/java/org/aibe4/dodeul/domain/board/service/BoardPostFileService.java
    ...
    public List<CommonFile> addFiles(Long memberId, Long postId, BoardPostFileCreateRequest request) {
        ...
        if (existing.size() + incoming > MAX_FILES_PER_POST) {
            throw new BoardPolicyException(
                ErrorCode.INVALID_INPUT_VALUE, "파일은 게시글당 최대 5개까지 등록할 수 있습니다.");
        }
        ...
        for (BoardPostFileCreateRequest.Item item : request.getFiles()) {
            CommonFile file =
                CommonFile.ofBoardPost(
                    postId, item.getFileUrl(), item.getOriginFileName(), item.getContentType(), item.getFileSize());
    ...
    public void deleteFile(Long memberId, Long postId, Long fileId) {
        ...
        CommonFile file =
            commonFileRepository
                ...
                .orElseThrow(
                    () -> new BoardPolicyException(ErrorCode.RESOURCE_NOT_FOUND, "파일을 찾을 수 없습니다."));
